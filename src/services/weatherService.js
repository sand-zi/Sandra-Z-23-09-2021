...
async function _getLocationByCoords(lat, lon) {
  try {
    const res = (await axios.get(`${GEOPOSITIONURL}apikey=${API_KEY}&q=${lat},${lon}`)) || null;

    return res
      ? _getFormatedLocation(res.data["LocalizedName"], res.data["Key"], res.data["Country"]["LocalizedName"])
      : { ...initialLocation };
  } catch (err) {
...
async function getForecasts(cityKey = "215854") {
  try {
    const res = await axios.get(`${FORECASTURL}/5day/${cityKey}?apikey=${API_KEY}&metric=true`);
...
async function getCityForecast(cityKey = initialLocation.key) {
  try {
    const res = await axios.get(`${FORECASTURL}/1day/${cityKey}?apikey=${API_KEY}&metric=true`);
    if (res.data["DailyForecasts"].length) {
...
async function getLocationsList(userInput) {
  try {
    const res = await axios.get(`${AUTOCOMPLETEURL}?apikey=${API_KEY}&q=${userInput}`);
